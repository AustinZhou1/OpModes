...
@TeleOp(name = "TeleOpMM", group = "TeleOpMM")
public class TeleOpMM extends LinearOpMode {
    ...
    private DcMotor mtrLS, mtrRS;               // Shooters for the particles
    private Servo sweeper;                      // Sweeps the particles for shooting
    ...
    private boolean x_button;
    private boolean y_button;
    private boolean right_joystick;
    ...
    //Sweeper Speed Values
    private double sweeperSpeed;
    ...
    private void initHardware() {
        ...
        mtrLS   = hardwareMap.dcMotor.get("mtrLS");
        mtrRS   = hardwareMap.dcMotor.get("mtrRS");
        ...
        mtrLS.setDirection(DcMotor.Direction.REVERSE);
        mtrRS.setDirection(DcMotor.Direction.FORWARD);
        ...
        mtrLS.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        mtrRS.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
    ...
    private void updateHardware() {
        ...
        mtrLS.setPower(shooterSpeed);
        mtrRS.setPower(shooterSpeed);
    ...
    private void mergeGamePads() {
        ...
        x_button = gamepad1.x || gamepad2.x;
        y_button = gamepad1.y || gamepad2.y;
        right_joystick = gamepad1.right_stick_button || gamepad2.right_stick_button;
        ...
        if (gamepad1.right_stick_button || gamepad2.right_stick_button) {
            right_joystick = gamepad1.right_stick_button;
        } else {
            right_joystick = gamepad2.right_stick_button;
        }
    ...
    private void shooter() {
        ...
        //sweeps in particles with servo sweeper motor
        while (y_button) {
            sweeperSpeed = 1.0;
        }


        // reverses servo sweeper motor
        while (right_joystick) {
            sweeperSpeed = 0.0;
        }
    ...
    private void executePushControl() {
        final double CENTER_POS = 0.5;  // Center of the beacon pusher
        final double LEFT_POS = 0.2;  // filter factor for nudge movement
        final double RIGHT_POS = 0.8;  // Right ready position
        final double LEFT_MAX = 0.05;  // Left ready position
        final double RIGHT_MAX = 0.95;
...
*/          telemetry.addData("Servo", "pusherPosition = %.1f", pusherPosition);
